...
class QuadricIouAssociator(DataAssociator):
    ...
    def associate(
        ...
        s = state.system
        n = state.this_step
        # retrieve all the pose and quadric values created so far
        ps, qs = ps_and_qs_from_values(state.system.estimates)
        # what should be the id of the new quadric
        ...
        # if no quadric has been made so far, then no need to check for assosciation
        if len(qs.values()) == 0:
        ...
        # a matrix with each row for each detection and each column for each existing quadric
        ious = np.zeros((len(n.detections), len(qs.values())))
        # check the IOU of the bounding box and the projected quadric at a particular pose 
        ...
        for di, qi in zip(dis, qis):
            # add the detection as a new quadric with a quadric key
            if ious[di, qi] < self.iou_thresh:
                n.detections[di].quadric_key = QI(next_q + i)
                i += 1
            else:
                # add the detection to the existing quadric key
