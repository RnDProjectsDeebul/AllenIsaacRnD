...
def initialise_quadric_from_depth(
    ...
    box = boxes[0]
    # only the first obj_pose is taken into account.
    pose = obs_poses[0]
...
def initialise_quadric_ray_intersection(
    ...
    # here all the poses are using for finding the centroid
    ...
    # returns only the correct centroid. the rotation and the size are a dumb guess.
    return gtsam_quadrics.ConstrainedDualQuadric(
        gtsam.Rot3(), gtsam.Point3(quadric_centroid), [1, 1, 0.1])


# cancel previous factors from current factors and add the new factors to the new NonlinearFactorGraph()
