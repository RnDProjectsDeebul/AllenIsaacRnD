...
class QuadricSlam:
    ...
    def guess_initial_values(self) -> None:
        ...
        # each of the 15 factors in a list.
        ...
        # s.estimates contain the initial pose.
        # s.estimates is a dictionary with key as posekey or quadrickey and value as the matrix  
        ...
        # now s.estimates contains 5 poses with key as pose key and value as the matrix
        ...
        for qbbs in [list(v) for k, v in groupby(bbs, _ok)]:
            # uses the initialise_quadric_ray_intersection for initializing the quadrics.
        ...
        # now s.estimates contains 7 elements in the dictionary where first 2 elements are the 2 quadrics and
        # the next 5 are the pose elements. The quadric factors are guessed values and the poses are exact.

    def spin(self) -> None:
        while not self.data_source.done():
            self.step()
            # the current code is in such a way that it visualize only after the entire data is read out,
            # the parameters are optimized, and then only. maybe can edit to visualize intermediate results.

        # initialize with some guessed values(exact pose but guessed quadric params except the quadric centroid)
        if self.state.system.optimiser_batch:
            ...
            # the above piece of code is similar to the last section in hello_manual_quadricslam.
            # r = gtsam.LevenbergMarquardtOptimizer(graph, values).optimize()
            # we will get the best optimised values in the s.estimates dictionary of length 7.
            # 5 poses and 2 quadrics

            # the callable function is visualise. that is, visualise the state of the system.
            if self.on_new_estimate:
                self.on_new_estimate(self.state)

    def step(self) -> None:
        # Setup state for the current step
        # s is the system global details
        s = self.state.system
        # prev_state is previous StepState
        p = self.state.prev_step
        # StepState(index i) - 0 for first or else increment by 1
        n = StepState(
            0 if self.state.prev_step is None else self.state.prev_step.i + 1)
        # this_step parameter in QuadricSlamState
        self.state.this_step = n

        # now on, n represents the current StepState
        ...
        # if optimiser_batch is false, we are optimising after each step
        # if optimiser_batch is ture, we are optimising after entire operation
    ...
    # cleaning up, reseting all the parameters
    def reset(self) -> None:
